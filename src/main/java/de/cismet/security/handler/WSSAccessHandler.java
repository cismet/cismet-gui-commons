...
public class WSSAccessHandler extends HTTPBasedAccessHandler {
    ...
    public InputStream doRequest(
        ...
        return new ByteArrayInputStream(
            accessor.doService(accessMethod, parameter.toString(), url.toString()).asBytes()
        );
    }
     ...
     * @throws  Exception  DOCUMENT ME!
     */
    private synchronized WSSAccessorDeegree createNewWSSAccessor(final URL url) throws Exception {
        if (log.isDebugEnabled()) {
    ...
    private synchronized void authenticate(final WSSAccessorDeegree accessor) throws Exception {
        if (!accessor.isSessionAvailable()) {
            if (log.isDebugEnabled()) {
                log.debug(
                    "Keine Session Informationen für url: " + accessor.getWSS() + " vorhanden --> Passwordfenster"
                ); // NOI18N
            }
    ...
    public String createGetRequest(final String request) throws Exception {
        ...
        sb.append(baseURL).append("?service=WSS&request=DoService&version=1.0.0&"); // NOI18N
    ...
    class WSSPasswordDialog extends PasswordDialog {
        ...
        public boolean authenticate(final String name, final char[] password, final String server) throws Exception {
            ...
            try {
                final AuthenticationMethod authMethod = new PasswordAuthenticationMethod(
